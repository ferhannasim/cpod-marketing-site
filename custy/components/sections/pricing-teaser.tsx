  ...
  return (
    <section className={scheme}>
      <Container className="py-16 md:py-24">
        ...
        <Reveal className="grid items-start gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {plans.map((plan) => (
            <div
              key={plan.name}
              className={cn(
            ...
            >
              ...
              <div className="mt-3 flex items-baseline gap-1.5">
                ...
                {plan.period ? <span className="text-[15px] text-muted">{plan.period}</span> : null}
              </div>
              <ul className="m-0 mt-4 list-none p-0">
                {plan.features.slice(0, 3).map((feature) => (
                  <li key={feature} className="mb-2 flex items-start gap-2.5 text-[15px] leading-[1.55] text-body">
